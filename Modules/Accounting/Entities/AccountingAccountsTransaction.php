...
class AccountingAccountsTransaction extends Model
{
    ...
    public static function createTransaction($data)
    {
        ...
        // Set the value of the foreign key column
        $transaction->business_location_id = $data['location_id'];
        ...
        $transaction->note = !empty($data['notes']) ? $data['notes'] : null;
    ...
    public static function updateOrCreateMapTransaction($data)
    {
        $transaction = AccountingAccountsTransaction::updateOrCreate(
            ...
            ['accounting_account_id' => $data['accounting_account_id'], 'amount' => $data['amount'], 
                'type' =>  $data['type'], 'sub_type' => $data['sub_type'], 'created_by' => $data['created_by'], 'operation_date' => $data['operation_date'] , 'notes' => $data['notes']
            ]
        );
    }
}
